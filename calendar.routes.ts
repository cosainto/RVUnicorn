...
router.get('/', authenticateToken, async (req: Request, res: Response) => {
  try {
    ...
    const events = await prisma.event.findMany({
      where: { OR: [{ organizerId: userId }, { attendees: { some: { userId } } }], startDate: { lt: end }, endDate: { gte: start } },
      include: {
    ...
    const items = [
      ...events.map(e => ({ id: e.id, type: 'EVENT', title: e.title, startDate: e.startDate, endDate: e.endDate, campground: e.campground, location: (e as any).location, isOrganizer: e.organizerId === userId, myAttendee: e.attendees.find(a => a.userId === userId) || null, attendees: e.attendees, color: '#f59e0b' })),
      ...trips.flatMap(t => (t.stays as any[]).map((s: any) => ({ id: s.id, tripId: t.id, type: 'STAY', title: t.name, startDate: s.startDate ?? t.startDate, endDate: s.endDate ?? t.endDate, campground: s.campground, isOrganizer: true, myAttendee: { confirmationNumber: s.confirmationNumber, siteNumber: s.siteNumber, userId }, attendees: [], color: '#34d399' }))),
    ];
    res.json({ items });
...
router.patch('/events/:eventId/reservation', authenticateToken, async (req: Request, res: Response) => {
  try {
    ...
    const updated = await prisma.eventAttendee.update({
      ...
      data: { ...(confirmationNumber !== undefined && { confirmationNumber }), ...(siteNumber !== undefined && { siteNumber }), ...(notes !== undefined && { notes }) },
    });
...
router.patch('/stays/:stayId/reservation', authenticateToken, async (req: Request, res: Response) => {
  try {
    ...
    const updated = await (prisma as any).stay.update({ where: { id: stayId }, data: { ...(confirmationNumber !== undefined && { confirmationNumber }), ...(siteNumber !== undefined && { siteNumber }) } });
...
router.post('/events/:eventId/attendees', authenticateToken, async (req: Request, res: Response) => {
  try {
    ...
    await prisma.notification.create({ data: { userId, type: 'EVENT_INVITE', content: `You were added to "${event.title}"`, link: `/trips/${eventId}`, category: 'SOCIAL' } });
    res.json({ attendee });
  } catch (e) { console.error(e); res.status(500).json({ error: 'Failed' }); }
});
