...
export async function ensureTriviaWeek(campgroundId: string, io?: any) {
  const { weekStart, weekEnd } = getWeekBounds(new Date());

  // Already has an active week this period — just make sure a question has been asked today
...
export async function askNextQuestion(io: any) {
  ...
  for (const room of rooms) {
    ...
    // Post Hitch recap of previous question's results
    if (questionNum > 1) {
      try {
        const prevQ = await prisma.triviaQuestion.findFirst({
          where: { weekId: week.id, dayOfWeek, questionNum: questionNum - 1 },
        });
        if (prevQ) {
          ...
          const fastest = correct.sort((a, b) => a.responseTime - b.responseTime)[0];

          let recapContext = `Q${questionNum - 1} results: ${correct.length} correct, ${wrong.length} wrong out of ${answers.length} total answers.`;
          if (fastest) recapContext += ` Fastest correct: ${fastest.user.firstName} in ${(fastest.responseTime / 1000).toFixed(1)}s.`;
          if (answers.length === 0) recapContext = `Nobody answered Q${questionNum - 1}! Crickets at the campfire.`;
          ...
          const recapRes = await anthropic.messages.create({
            ...
            messages: [{
              role: 'user',
              content: `You are Hitch, the RV Unicorn campfire host. ${recapContext} Write ONE short sentence reacting to these results. Be funny, campy, family-safe. Tease slowpokes or hype fast answers.`,
            }],
          });
          const recap = recapRes.content[0].type === 'text' ? recapRes.content[0].text.trim() : null;
          if (recap) {
            await prisma.campfireMessage.create({
              data: { roomId: room.id, isHitch: true, content: recap },
            });
          }
        }
      } catch {}
    ...
    // Hitch trash talk — generated by Claude Haiku for flavor
    try {
      const checkedInCount = await prisma.checkIn.count({ where: { campgroundId: room.campgroundId, isActive: true } });
      const leaderboard = await prisma.triviaLeaderboard.findMany({
        where: { weekId: week.id },
        orderBy: { totalPoints: 'desc' },
        take: 3,
        include: { user: { select: { firstName: true } } },
      });
      const topNames = leaderboard.map(l => l.user.firstName).filter(Boolean);

      let hitchContext = `Question ${questionNum} of 10 just dropped. ${checkedInCount} campers at the fire.`;
      if (questionNum === 1) hitchContext = `Trivia session just started! ${checkedInCount} campers at the fire. Get them hyped!`;
      if (questionNum === 10) hitchContext = `This is the LAST question! Everything on the line.${topNames.length ? ` Current leader: ${topNames[0]}.` : ''}`;
      if (topNames.length && questionNum > 1 && questionNum < 10) hitchContext += ` Leaderboard: ${topNames.join(', ')}.`;

      const hitchRes = await anthropic.messages.create({
        model: 'claude-haiku-4-5-20251001',
        max_tokens: 100,
        messages: [{
          role: 'user',
          content: `You are Hitch, the fun RV Unicorn campfire host. ${hitchContext} Write ONE short (1-2 sentences) fun trash-talk message for the campfire chat. Campy/outdoorsy humor, family safe, no profanity. Do NOT include the question or answers. Just hype, tease, or encourage.`,
        }],
      });
      const hitchMsg = hitchRes.content[0].type === 'text' ? hitchRes.content[0].text.trim() : null;
      if (hitchMsg) {
        await prisma.campfireMessage.create({
          data: { roomId: room.id, isHitch: true, content: hitchMsg },
        });
      }
    } catch {
      // Fallback if Haiku fails
      await prisma.campfireMessage.create({
        data: { roomId: room.id, isHitch: true, content: questionNum === 10 ? `🔥 LAST QUESTION! Everything rides on this one!` : `🎯 Q${questionNum} is live — who's got it?` },
      });
    }
...
export async function postDailyLeaderboard(io: any) {
  ...
  for (const room of rooms) {
    ...
    // Hitch dramatic session-end announcement
    try {
      const topName = board[0]?.user?.firstName || 'someone';
      const topPts = board[0]?.totalPoints || 0;
      const sessionEndRes = await anthropic.messages.create({
        model: 'claude-haiku-4-5-20251001',
        max_tokens: 100,
        messages: [{
          role: 'user',
          content: `You are Hitch, the RV Unicorn campfire host. The trivia session just ended! ${board.length} campers played. Leader: ${topName} with ${topPts} pts. Write 1-2 dramatic sentences announcing results are in and the leaderboard is about to drop. Campy/outdoorsy humor, family-safe.`,
        }],
      });
      const endMsg = sessionEndRes.content[0].type === 'text' ? sessionEndRes.content[0].text.trim() : null;
      if (endMsg) {
        await prisma.campfireMessage.create({
          data: { roomId: room.id, isHitch: true, content: endMsg },
        });
      }
    } catch {}
