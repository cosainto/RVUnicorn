...
const PROGRESS_FILE = path.join(__dirname, 'import-progress-balanced.json');
...
// BALANCED QUALITY VALIDATION - More permissive
function isValidName(name: string | undefined): boolean {
  if (!name) return false;
  
  const trimmed = name.trim();
  
  // Reject only obvious numbered names like "#1", "#2"
  if (/^#\d+$/.test(trimmed)) return false;
  
  // Reject very short names (less than 2 chars)
  if (trimmed.length < 2) return false;
  
  // Accept everything else - let users see the data!
  return true;
}
...
function hasMinimumQuality(element: OSMElement): boolean {
  ...
  // Only reject if BOTH name is bad AND coordinates are bad
  // If coordinates are good, import it even with a generic name
  const hasValidCoords = isValidCoordinates(lat, lon);
  const hasValidName = isValidName(name);
  
  // Must have valid coordinates (this is critical)
  if (!hasValidCoords) return false;
  
  // Must have SOME name (even if generic)
...
async function fetchCampgroundsForRegion(bbox: string, regionName: string): Promise<OSMElement[]> {
  ...
  try {
    ...
    console.log(`  ${qualityElements.length} have valid coordinates and names`);
    console.log(`  ${elements.length - qualityElements.length} rejected (missing data)`);
...
async function importCampground(element: OSMElement, defaultState: string): Promise<boolean> {
  try {
    const name = element.tags?.name || 'Unnamed Campground';
    
    const lat = element.lat || element.center?.lat;
    const lon = element.lon || element.center?.lon;
    if (!lat || !lon) return false;

    // Create a more unique slug by including OSM ID
    const baseName = name.toLowerCase()
      .replace(/[^a-z0-9\s-]/g, '')
      .replace(/\s+/g, '-')
      .substring(0, 80);
    const slug = `${baseName}-${element.id}`;

    // Check if already exists by slug or exact coordinates
...
async function main() {
  console.log('🏕️  BALANCED OSM Campground Import');
  console.log('📊 This imports campgrounds with:');
  console.log('   ✅ Valid coordinates (required)');
  console.log('   ✅ Any name (even generic ones)');
  console.log('   ❌ Only rejects: numbered names (#1, #2) and missing data');
  console.log('');
  ...
  console.log(`⏭️  Skipped incomplete: ${progress.skippedLowQuality}`);
  console.log('');
  ...
  if (remainingRegions.length === 0) {
    ...
    console.log(`   - Total incomplete skipped: ${progress.skippedLowQuality}`);
  ...
  for (let i = 0; i < remainingRegions.length; i++) {
    ...
    console.log(`  ✅ Imported ${imported} campgrounds from ${region.name}`);
    ...
    // Wait 3 seconds between regions
