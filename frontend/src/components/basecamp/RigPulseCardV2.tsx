...
function SkeletonCard() {
  return (
    <div style={{ background: CN.card, borderRadius: 16, padding: 20, maxHeight: 250, overflow: 'hidden' }}>
      ...
      <div style={{ width: '60%', height: 32, borderRadius: 6, background: CN.border, marginBottom: 12, animation: 'pulse 1.5s ease-in-out infinite' }} />
      <div style={{ width: '80%', height: 10, borderRadius: 4, background: CN.border, marginBottom: 16, animation: 'pulse 1.5s ease-in-out infinite' }} />
...
function ActiveTripState({ data }: { data: RigPulseData }) {
  ...
  return (
    <div style={{ background: CN.card, borderRadius: 16, padding: 16, borderLeft: `3px solid ${CN.orange}`, maxHeight: 250, overflow: 'hidden' }}>
      ...
      <div style={{ fontSize: 12, color: CN.muted, marginBottom: 12 }}>{trip.totalMiles.toLocaleString()} Miles &middot; {trip.totalNights} Nights &middot; {trip.statesVisited} States</div>
...
function IdleState({ data }: { data: RigPulseData }) {
  ...
  if (data.newFollowers.length > 0) {
    ...
    items.push(
      <div key="followers" style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, color: CN.muted, padding: '3px 0' }}>
        <AvatarStack avatars={data.newFollowers} size={20} />
        <span>{first.firstName}{rest > 0 ? ` and ${rest} other${rest > 1 ? 's' : ''}` : ''} followed your rig</span>
      </div>
    );
  }
  ...
  return (
    <div style={{ background: CN.card, borderRadius: 16, padding: 16, borderLeft: '3px solid rgba(232,168,56,0.4)', maxHeight: 250, overflow: 'hidden' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        <Link to="/rig/edit" style={{ fontSize: 12, color: CN.gold, textDecoration: 'none' }}>Add Rig Photo</Link>
      </div>
      <HitchNudge data={data} />
    </div>
  );
